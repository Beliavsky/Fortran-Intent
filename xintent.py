...
BLOCK_IF_START_RE = re.compile(r"^\s*if\s*\(.*\)\s*then\b", re.IGNORECASE)
BLOCK_ELSEIF_RE = re.compile(r"^\s*else\s*if\s*\(.*\)\s*then\b", re.IGNORECASE)
...
def is_whole_dummy_write(lhs: str, dummy: str, is_array: bool) -> bool:
    ...
    if not is_array:
        return False
    m = re.match(rf"^{re.escape(d)}\s*\((.*)\)$", lhs_s, re.IGNORECASE)
    if not m:
        return False
    parts = [p.strip() for p in re.split(r",(?![^()]*\))", m.group(1))]
    if not parts:
        return False
    return all((p == ":" or re.fullmatch(r":\s*:\s*1", p) is not None) for p in parts)


def parse_rank1_decl_bounds(entity: str, dummy: str) -> Optional[Tuple[int, int]]:
    """Return rank-1 integer bounds for one declared entity when statically known."""
    ent = entity.strip().lower()
    d = dummy.lower()
    m = re.match(rf"^{re.escape(d)}\s*\((.*)\)$", ent, re.IGNORECASE)
    if not m:
        return None
    ...
    if re.fullmatch(r"[0-9]+", spec):
        n = int(spec)
        return (1, n)
    m_rng = re.fullmatch(r"\s*([0-9]+)\s*:\s*([0-9]+)\s*", spec)
    if m_rng:
...
def parse_dummy_decl_shape(entity: str, dummy: str) -> Tuple[int, Optional[Tuple[int, int]]]:
    """Return dummy rank and static rank-1 bounds when available."""
    ent = entity.strip().lower()
    d = dummy.lower()
    m = re.match(rf"^{re.escape(d)}\s*\((.*)\)$", ent, re.IGNORECASE)
    if not m:
    ...
    rank = len(parts) if parts else 0
    if rank == 1:
        return 1, parse_rank1_decl_bounds(entity, dummy)
    return rank, None


def parse_rank1_write_coverage(lhs: str, dummy: str, bounds: Optional[Tuple[int, int]]) -> Optional[Set[int]]:
    """Return exact covered indices for a rank-1 dummy write when statically known."""
    if bounds is None:
        return None
    lo_b, hi_b = bounds
    lhs_s = lhs.strip().lower()
    ...
    if not m_rng:
        return None
    lo = int(m_rng.group(1)) if m_rng.group(1) else lo_b
    hi = int(m_rng.group(2)) if m_rng.group(2) else hi_b
    st = int(m_rng.group(3)) if m_rng.group(3) else 1
    if st <= 0:
        return None
    lo = max(lo, lo_b)
    hi = min(hi, hi_b)
    if lo > hi:
        return set()
    return set(range(lo, hi + 1, st))


def summarize_uncovered_indices(bounds: Tuple[int, int], covered: Set[int]) -> str:
    """Describe uncovered rank-1 indices/ranges inside bounds."""
    lo_b, hi_b = bounds
    missing = [i for i in range(lo_b, hi_b + 1) if i not in covered]
    if not missing:
        return ""
    ...
    if len(runs) == 1 and runs[0][0] == runs[0][1]:
        return f"element {runs[0][0]} not set"
    parts = []
    for lo, hi in runs:
        parts.append(f"{lo}" if lo == hi else f"{lo}:{hi}")
    return f"indices {', '.join(parts)} not set"


def parse_rank1_write_descriptor(lhs: str, dummy: str) -> Optional[str]:
    """Describe a definite rank-1 write when full bounds are unknown."""
    lhs_s = lhs.strip().lower()
    d = dummy.lower()
    if lhs_s == d:
        return "whole array is definitely set"
    m = re.match(rf"^{re.escape(d)}\s*\((.*)\)$", lhs_s, re.IGNORECASE)
    if not m:
        return None
    spec = m.group(1).strip()
    if "," in spec:
        return None
    if re.fullmatch(r"[0-9]+", spec):
        return f"only element {spec} is definitely set; other elements may be unset"
    m_rng = re.fullmatch(
        r"\s*([0-9]*)\s*:\s*([0-9]*)(?:\s*:\s*([0-9]+))?\s*",
        spec,
    )
    ...
    if lo and hi:
        if lo == hi:
            return f"only element {lo} is definitely set; other elements may be unset"
        return f"only section {lo}:{hi} is definitely set; other elements may be unset"
    if lo and not hi:
        return f"only section {lo}: is definitely set"
    if (not lo) and hi:
        return f"only section :{hi} is definitely set; elements after {hi} may be unset"
    return "whole array is definitely set"


def parse_rank2_write_descriptor(lhs: str, dummy: str) -> Optional[str]:
    """Describe a definite rank-2 write pattern conservatively."""
    lhs_s = lhs.strip().lower()
    d = dummy.lower()
    if lhs_s == d:
        return "whole array is definitely set"
    ...
    if p1 == ":" and p2 == ":":
        return "whole array is definitely set"
    if re.fullmatch(r"[0-9]+", p1) and re.fullmatch(r"[0-9]+", p2):
        return f"only element ({p1},{p2}) is definitely set; other elements may be unset"
    if re.fullmatch(r"[0-9]+", p1) and p2 == ":":
        return f"only row {p1} is definitely set; other elements may be unset"
    if p1 == ":" and re.fullmatch(r"[0-9]+", p2):
        return f"only column {p2} is definitely set; other elements may be unset"
    if ":" in p1 or ":" in p2:
        return f"only section ({p1},{p2}) is definitely set; other elements may be unset"
    return None


def conditionalize_write_descriptor(desc: str) -> str:
    """Convert a definite-write descriptor into a conditional-write descriptor."""
    if not desc:
        return desc
    if desc == "whole array is definitely set":
        return "whole array may be set conditionally"
    return desc.replace(" is definitely set", " may be set")
...
def print_itemized_summary(items: List[IntentSuggestion], label: str) -> None:
    """Print one warning per line."""
    if not items:
        return
    n_functions = sum(1 for s in items if s.proc_kind == "function")
    n_subroutines = sum(1 for s in items if s.proc_kind == "subroutine")
    nfiles = len({s.filename for s in items})
    fun_word = "function" if n_functions == 1 else "functions"
    sub_word = "subroutine" if n_subroutines == 1 else "subroutines"
    file_word = "source file" if nfiles == 1 else "source files"
    print(
    ...
    for s in sorted(items, key=lambda x: (x.filename.lower(), x.proc_kind, x.proc_name.lower(), x.dummy.lower())):
        detail = f" [{s.detail}]" if s.detail else ""
        print(f"{s.filename} {s.proc_kind} {s.proc_name}:{s.dummy}{detail}")
...
def collect_unset_intent_out_args(finfo: fscan.SourceFileInfo) -> List[IntentSuggestion]:
    """Warn for declared INTENT(OUT) dummies that are never wholly assigned."""
    out: List[IntentSuggestion] = []
    sorted_procs = sorted(finfo.procedures, key=lambda p: p.start)
    all_stmts = fscan.iter_fortran_statements(finfo.parsed_lines)
    for pidx, proc in enumerate(sorted_procs):
        ...
        next_start = sorted_procs[pidx + 1].start if pidx + 1 < len(sorted_procs) else (len(finfo.parsed_lines) + 1)
        body_stmts = statements_in_range(all_stmts, proc.start, next_start)
        decl: Dict[str, Tuple[int, bool, int, Optional[Tuple[int, int]]]] = {}
        dummy_is_array: Dict[str, bool] = {d: False for d in proc.dummy_names}
        whole_writes: Set[str] = set()
        partial_writes: Dict[str, Set[int]] = {}
        partial_descriptions: Dict[str, List[str]] = {}
        conditional_descriptions: Dict[str, List[str]] = {}
        if_depth = 0

        for ln, code in body_stmts:
            low = code.lower().strip()
            if low and TYPE_DECL_RE.match(low) and not low.startswith("procedure"):
                declared = parse_declared_names_any(low)
                arr_flags = parse_declared_array_flags(low)
                intent = parse_decl_intent(low)
                rhs_decl = low.split("::", 1)[1] if "::" in low else low
                ents = split_decl_entities(rhs_decl)
                for d in proc.dummy_names:
                    if d in declared and intent == "out" and d not in decl:
                        rank = 0
                        bounds = None
                        for ent in ents:
                            m_ent = re.match(r"^\s*([a-z][a-z0-9_]*)", ent, re.IGNORECASE)
                            if m_ent and m_ent.group(1).lower() == d:
                                rank, bounds = parse_dummy_decl_shape(ent, d)
                                break
                        decl[d] = (ln, True, rank, bounds)
                    if d in arr_flags and arr_flags[d]:
                        dummy_is_array[d] = True
                continue

            low_nolab = strip_statement_label(low)
            if END_IF_RE.match(low_nolab):
                if_depth = max(0, if_depth - 1)
            had_one_line_if = strip_one_line_if_prefix(low_nolab) != low_nolab
            had_one_line_where = strip_one_line_where_prefix(low_nolab) != low_nolab
            in_conditional_block = if_depth > 0
            is_conditional_write = had_one_line_if or had_one_line_where or in_conditional_block
            low_exec = strip_one_line_if_prefix(low_nolab)
            low_exec = strip_one_line_where_prefix(low_exec)
            ...
            if asn is not None:
                lhs, _rhs = asn
                lhs_base = fscan.base_identifier(lhs)
                if lhs_base in proc.dummy_names and lhs_base in decl:
                    if is_whole_dummy_write(lhs, lhs_base, dummy_is_array.get(lhs_base, False)):
                        if is_conditional_write:
                            bucket = conditional_descriptions.setdefault(lhs_base, [])
                            desc = "whole array may be set conditionally"
                            if desc not in bucket:
                                bucket.append(desc)
                        else:
                            whole_writes.add(lhs_base)
                    else:
                        _decl_line, _intent_out, rank, bounds = decl[lhs_base]
                        if rank == 1:
                            covered = parse_rank1_write_coverage(lhs, lhs_base, bounds)
                            if covered is not None and (not is_conditional_write):
                                partial_writes.setdefault(lhs_base, set()).update(covered)
                            desc = parse_rank1_write_descriptor(lhs, lhs_base)
                        elif rank == 2:
                            desc = parse_rank2_write_descriptor(lhs, lhs_base)
                        else:
                            desc = None
                        if desc:
                            if is_conditional_write:
                                desc = conditionalize_write_descriptor(desc)
                                bucket = conditional_descriptions.setdefault(lhs_base, [])
                                if desc not in bucket:
                                    bucket.append(desc)
                            elif desc != "whole array is definitely set":
                                bucket = partial_descriptions.setdefault(lhs_base, [])
                                if desc not in bucket:
                                    bucket.append(desc)
            ...
            if READ_START_RE.match(low_exec):
                for obj in extract_read_iolist_bases(low_exec):
                    if obj in decl:
                        if is_conditional_write:
                            bucket = conditional_descriptions.setdefault(obj, [])
                            desc = "whole array may be set conditionally"
                            if desc not in bucket:
                                bucket.append(desc)
                        else:
                            whole_writes.add(obj)

            if "=>" in low_exec:
                for d in decl:
                    if re.search(rf"^\s*{re.escape(d)}\s*=>", low_exec):
                        if is_conditional_write:
                            bucket = conditional_descriptions.setdefault(d, [])
                            desc = "whole array may be set conditionally"
                            if desc not in bucket:
                                bucket.append(desc)
                        else:
            ...
            if BLOCK_IF_START_RE.match(low_nolab) and not had_one_line_if:
                if_depth += 1
            elif BLOCK_ELSEIF_RE.match(low_nolab):
                if_depth = max(if_depth, 1)

        for d in sorted(decl):
            if d in whole_writes:
                continue
            decl_line, _, rank, bounds = decl[d]
            detail = ""
            if rank == 1 and bounds is not None:
                detail = summarize_uncovered_indices(bounds, partial_writes.get(d, set()))
            if not detail and partial_writes.get(d):
                detail = "not wholly set"
            if not detail and partial_descriptions.get(d):
                if len(partial_descriptions[d]) == 1:
                    detail = partial_descriptions[d][0]
                else:
                    detail = "definitely set: " + "; ".join(partial_descriptions[d])
            if conditional_descriptions.get(d):
                cdesc = (
                    conditional_descriptions[d][0]
                    if len(conditional_descriptions[d]) == 1
                    else "may be set conditionally: " + "; ".join(conditional_descriptions[d])
                )
                detail = f"{detail}; {cdesc}" if detail else cdesc
            out.append(
                IntentSuggestion(
                    filename=fscan.display_path(finfo.path),
                    proc_kind=proc.kind,
...
def main() -> int:
    ...
    if args.warn_unset_intent_out:
        source_files, any_missing = fscan.load_source_files(args.fortran_files)
        if not source_files:
            return 2 if any_missing else 1
        final_ordered, _ = fscan.order_files_least_dependent(source_files)
        unset_summary: Dict[Tuple[str, str], List[str]] = {}
        unset_items: List[IntentSuggestion] = []
        unset_count = 0
        for finfo in final_ordered:
            unset = collect_unset_intent_out_args(finfo)
            for s in unset:
                add_summary_item(unset_summary, s)
                unset_items.append(s)
                unset_count += 1
        if unset_count > 0:
            print_itemized_summary(unset_items, label="with declared intent(out) arguments not wholly set")
        else:
            print("\nAll declared intent(out) arguments appear to be wholly set.")
